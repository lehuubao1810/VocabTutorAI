...
export const UserModal: React.FC<Props> = (props) => {
  console.log(props);
  return (
    <div className="absolute top-24 right-0 bg-white shadow-lg rounded-lg p-4 pt-2 z-50">
      <div className="flex flex-col items-center justify-center gap-2 mb-4 p-2 border-b-slate-300 border-b-2">
        ...
        {props.user?.photoURL ? (
          <img
            ...
            alt=""
            className="w-20 h-20 rounded-full mb-2 mt-1"
          />
        ) : (
          <AvatarUser username={props.user?.username} isClickable={false} />
        )}
